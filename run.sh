...
# 并行检查所有模板
for template_key in $(seq 1 12); do
    # 解析模板和参数
    template_info=$(echo "${url_templates[$template_key]}" | tr '|' '\n')
    template=$(echo "$template_info" | sed -n '1p')
    param1_type=$(echo "$template_info" | sed -n '2p')
    param2_type=$(echo "$template_info" | sed -n '3p')
    param3_type=$(echo "$template_info" | sed -n '4p')
...
found_count=0
for template_key in $(seq 1 12); do
    eval "url_value=\$template_valid_urls_${template_key}"
    if [ -n "$url_value" ]; then
...
if [ $found_count -eq 0 ]; then
    ...
    for template_key in $(seq 1 12); do
        ...
        case $template_key in
            1)
                ...
                eval "template_valid_urls_${template_key}=$(printf "$template" "$currentyear" "$currentmonth" "$date_full_default")"
                ;;
            2)
                # 模板2: https://nodefree.githubrowcontent.com/%s/%s/%s.yaml|year|month_padded|date_full
                eval "template_valid_urls_${template_key}=$(printf "$template" "$currentyear" "$currentmonth_padded" "$date_full_default")"
                ;;
            3)
                # 模板3: https://free.datiya.com/uploads/%s-clash.yaml|date_full
                eval "template_valid_urls_${template_key}=$(printf "$template" "$date_full_default")"
                ;;
            *)
                ...
                if [ -z "$param2_type" ] && [ -z "$param3_type" ]; then
                    ...
                    eval "template_valid_urls_${template_key}=$(printf "$template" "$param1_val" "$param2_val" "$param3_val")"
                    ...
                    eval "template_valid_urls_${template_key}=$(printf "$template" "$param1_val" "$param2_val")"
                fi
                ;;
        esac
    done
    ...
    for template_key in $(seq 1 12); do
        eval "url_value=\$template_valid_urls_${template_key}"
        if [ -n "$url_value" ]; then
            echo "使用模板[$template_key]: $url_value"
        fi
    done
fi
...
echo "cat ./clash_subscribe_url.txt"#!/bin/bash
#########################################################
# 节点订阅自动获取脚本 - 并行模板版本
# 功能：自动查找可用的节点URL并生成订阅
# 特点：并行检测、超时控制、多模板支持
